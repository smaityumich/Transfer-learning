...
n_source, alpha, gen_error = np.zeros(len(error)), np.zeros(len(error)), np.zeros(len(error))

for i in range(len(error)):
    par, gen_error[i] = error[i]
    n_source[i], _, alpha[i] = par
    
    
errors = pd.DataFrame(np.array([n_source, alpha, gen_error]).T, columns = ['n_source', '$\\alpha$', 'gen_error'])
